...
- **ContentFilter** (`src/content-filter/`) is a self-contained module (importing only `@grove/shared` types and `MediaIndex`) designed to be liftable into a separate project. Two tiers:
  ...
  - _Local NSFW pre-classifier_ (`signals/nsfw-preprocess.ts`, `signals/nsfw-infer.ts`, `signals/local-nsfw-runtime.ts`) — a self-hosted opennsfw2 model (`server/models/opennsfw2.onnx`, Apache 2.0; see `server/models/convert_opennsfw2_to_onnx.py` for how it was converted from the original Keras weights) run via `onnxruntime-node`, with `sharp` replicating opennsfw2's preprocessing (resize/crop/BGR-mean-subtract — parity-tested against a Python-generated fixture in `server/test/fixtures/`, since sharp and PIL's resize kernels aren't bit-identical). Currently **shadow-mode only**: when enabled it runs alongside every Vision call and logs a comparison (`local-nsfw shadow comparison`) but never changes the persisted verdict — `signals/local-nsfw.ts` has the clean/nsfw/uncertain thresholds intended for once it's promoted to a real pre-filter gate in front of Vision. `LOCAL_NSFW_MODEL_PATH` unset disables it entirely.
...
| `LOCAL_NSFW_MODEL_PATH`        | (unset)                        | Enables shadow-mode local NSFW pre-classification                     |
